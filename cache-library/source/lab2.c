...
static int lab2OpenApple(const char *path) {
...
#if defined(__APPLE__)
    return lab2OpenApple(path);
#elif defined(__linux__) && defined(O_DIRECT)
    return lab2OpenLinux(path);
#else
    return lab2OpenUnknown(path);
#endif
}
...
static ssize_t lab2ReadWrite(int fd, void *buf, size_t count, Mode mode) {
    pthread_mutex_lock(&commonMutex);
...
ssize_t lab2Read(int fd, void *buf, size_t count) {
    return lab2ReadWrite(fd, buf, count, READ_MODE);
}

ssize_t lab2Write(int fd, void *buf, size_t count) {
    return lab2ReadWrite(fd, buf, count, WRITE_MODE);
}
