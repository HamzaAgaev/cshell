...
void testOneFileEditedByManyFd() {
    initializeLRUCache();
    int fd = cioOpen(TEST_FILE_NAME);
    int fdArray[FD_COUNT];
    char expectedText[FD_COUNT + 1];
    for (char i = 0; i < FD_COUNT; i++) {
        char symbol = 'a' + i;
        fdArray[i] = cioOpen(TEST_FILE_NAME);
        cioLseek(fdArray[i], i, CIO_SEEK_SET);
        cioWrite(fdArray[i], &symbol, 1);
        expectedText[i] = symbol;
    }
    expectedText[FD_COUNT] = '\0';
    char writtenText[FD_COUNT + 1];
    cioLseek(fd, 0, CIO_SEEK_SET);
    cioRead(fd, writtenText, FD_COUNT);
...
void *threadFunc(void *arg) {
    ...
    cioLseek(arg_.fd, arg_.threadId, CIO_SEEK_SET);
    assert(cioWrite(arg_.fd, &symbol, 1) != -1);
